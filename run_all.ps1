...
Get-Content $NodesFile | ForEach-Object {
    ...
    foreach ($heap in $heapTypes) {
        foreach ($graph in $graphTypes) {
            ...
            $args = @("run", $ZigFile, "--", $Edges, $heap, $graph, $from, $to)
            $errfile = "$outfile.err"
            ...
            $proc = Start-Process -FilePath "zig" -ArgumentList $args -NoNewWindow -Wait -PassThru -RedirectStandardOutput $outfile -RedirectStandardError $errfile
            $sw.Stop()
